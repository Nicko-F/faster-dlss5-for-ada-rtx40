...
function Save-FixtureManifest {
    ...
    @{schemaVersion=1;profiles=@{'1920x1080'=@{
        integratedCount=43;manifestSha256=(Get-Sha (Join-Path $profile 'manifest.json'));
        addonSha256=$files['ada-nr.addon64']
    };'3840x2160'=@{integratedCount=39;manifestSha256=('a'*64);addonSha256=('b'*64)}}} |
        ConvertTo-Json -Depth 6 | Set-Content (Join-Path $PackageRoot 'compatibility/profiles.lock.json')
}
try {
    $baseFiles=@{}
    ...
    $profile=Join-Path $PackageRoot 'profiles/1920x1080'
    ...
    $fixtureNames+=@(0..42 | ForEach-Object {'bundle/integrated-{0:d2}.cubin' -f $_})
    ...
    $manifest=@{schemaVersion=1;resolution='1920x1080';mode=2;files=$files}
    Save-FixtureManifest
    Require ((Get-GameBin (Split-Path (Split-Path $bin))) -eq $bin) 'Game root resolution failed'
    Expect-Stop {Get-Profile '3840x2160'} 'payload is not included'
    Install-Profile $bin '1920x1080'
    Expect-Stop {Install-Profile $bin '1920x1080'} 'already exists'
    ...
    [IO.File]::WriteAllText((Join-Path $bundle 'ada-nr-frames.csv'),"$header`n0,1,145,0,0,1920,1080,0,0,2,28`n1,1,145,0,0,1920,1080,2,0,0,28`n")
    Show-Status $bin
    ...
    Expect-Stop {Get-Profile '1920x1080'} 'Payload mismatch'
    ...
    Assert-Baseline $bin
    $saved=$files['bundle/integrated-42.cubin']
    $manifest.files.Remove('bundle/integrated-42.cubin')
    $manifest|ConvertTo-Json|Set-Content (Join-Path $profile 'manifest.json')
    Expect-Stop {Get-Profile '1920x1080' $false} 'not the pinned tested identity'
    Save-FixtureManifest
    Expect-Stop {Get-Profile '1920x1080' $false} 'Profile file set'
    $manifest.files['bundle/integrated-43.cubin']=$saved
    Save-FixtureManifest
    Expect-Stop {Get-Profile '1920x1080' $false} 'Profile file set'
    $manifest.files['bundle/integrated-42.cubin']=$saved
    Save-FixtureManifest
    Expect-Stop {Get-Profile '1920x1080' $false} 'Profile file set'
    $manifest.files.Remove('bundle/integrated-43.cubin')
    ...
    $record=@{schemaVersion=1;resolution='1920x1080';addonSha256=$files['ada-nr.addon64'];packageRoot='old-moved-package'}
    ...
    $record.schemaVersion=1;$record.addonSha256=$files['ada-nr.addon64'];$record|ConvertTo-Json|Set-Content $statePath

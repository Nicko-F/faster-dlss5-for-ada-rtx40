...
1. [Summary and optimization objectives](#overview)
2. [Architecture and porting costs](#architecture)
3. [Asynchronous movement and compute pipelines](#async-pipeline)
4. [Register lifetimes and spill/reload elimination](#register-lifetimes)
5. [Tiling and data reuse inside fused kernels](#tiling-reuse)
6. [Lower register budgets for more CTA concurrency](#cta-concurrency)
7. [Shared-memory spilling and on-chip allocation](#shared-spilling)
8. [Final optimization combinations by module](#module-composition)
9. [Runtime integration and general dimensions](#runtime-integration)
10. [Validation methods and performance results](#validation-results)

<a id="overview"></a>

## 1. Summary and optimization objectives
...
reuse inside fused FFNs, lower register budgets for more CTA capacity, and selection
of shared spilling and block compilation constraints. Paired RTX 4080 measurements reduced pure NR
time by **5.35% / 9.11% / 10.30%** at 1080p / 1440p / 4K; independent 4K processes
measured **8.16%**. Section 10 gives the setup and game results.

Hardware mechanisms cite NVIDIA documentation, architecture cites community research, and resource/performance figures come from this project. Sections 3–7 cover five optimization methods, Section 8 maps their combinations to modules, and Sections 9–10 cover integration and validation.

<a id="architecture"></a>

## 2. Architecture and porting costs

### 2.1 Model background and terminology
...
### 2.2 Copy and synchronization capabilities
...
copies: the 512-byte case in Section 3 is linear.
...
completion dependencies. A bulk copy need not involve multiple CTAs. The example in Section 3
...
### 2.3 Uniform execution and ordinary-register pressure
...
controls in Section 4 directly establish how register crowding and load scheduling produce
...
### 2.4 Resource limits and cost model
...
<a id="async-pipeline"></a>

## 3. Asynchronous movement and compute pipelines
...
### 3.3 Resource changes and costs
...
Module results appear in Section 8; current full-network results are in the
[benchmark report](benchmarks.md).

<a id="register-lifetimes"></a>

## 4. Register lifetimes and spill/reload elimination
...
QKV retains its 168-register budget and loads A fragments in batches near their current consumers, reusing registers whose previous values are no longer live.
...
### 4.3 Resource changes and costs
...
allocation. These changes remove the corresponding spill requirement; Section 7
addresses selecting storage for remaining spills.

### 4.4 Correctness

Load transformations preserve the read set, matrix operands, quantization and each result's accumulation order. Crossed instructions are checked for consumers, address changes, shared writes and synchronization boundaries. Outputs, scratch and guards are validated using Section 10's protocol.
...
Batched loading is retained in the QKV and 512 projection combinations. The SM120/SM89 controls above explain resource changes; timing measures complete fused kernels and module combinations. Section 8 summarizes module measurements and Section 10 reports full inference.

<a id="tiling-reuse"></a>

## 5. Tiling and data reuse inside fused kernels
...
Adjacent output slices originally reload the same A input fragment. These shared reads consume instructions and memory-access resources even in the absence of local spills.
...
### 5.3 Resource changes and costs
...
Each result retains its K accumulation order, contraction slice order,
FP8/FP16 conversions and output layout.

Boundary resampling, surrounding projection work and synchronization retain their original semantics.

### 5.5 Measured results

N64 input reuse is retained in 256 chained and selected input/output boundaries alongside Sections 4 and 6's methods. Section 8 reports their local aggregates; equal channel counts do not require identical tiling.

<a id="cta-concurrency"></a>

## 6. Lower register budgets for more CTA concurrency
...
**This method lowers the per-thread register cap, accepting local spills
...
### 6.3 Resource changes and costs
...
| Compilation policy with original sampling order | Actual registers/thread | Local stack | Reported spill stores/loads | Shared/CTA |
|---|---:|---:|---:|---:|
| 168-register control | 168 | 0 B | 0 / 0 B | 2,048 B |
| Retained 128-register implementation | 128 | 80 B | 80 / 80 B | 2,048 B |
...
Budget changes preserve the grid, runtime threads per CTA, numerical operations and dependencies. Pre retains sampling coordinates and texture-access order; 256 chained follows Section 5's N64 ordering checks. Capacity queries, compiled resources and output agreement are recorded separately.
...
<a id="shared-spilling"></a>

## 7. Shared-memory spilling and on-chip allocation
...
### 7.3 Resource changes and costs
...
Spill on/off controls use the same compilation constraints. Outputs, synchronization state and guards are checked before complete-kernel replay. Sections 9–10 cover Post surface and history validation.
...
**Fixed-block code generation in pooling.** Spill pragma on/off produces the same
binary in this control, identifying the incremental gain with code generation for
the fixed block.
...
<a id="module-composition"></a>

## 8. Final optimization combinations by module

The current combination covers encoder, ViT, decoder and temporal Pre/Post.
...
### 8.1 Map methods to modules

| Execution region | Retained method combination | Resources and selection |
|---|---|---|
| Encoder/decoder 256 chained | N64 reuse (Section 5) + lower register budget (Section 6) | 128 registers, 64-byte local stack; CTA capacity 1 → 2/SM |
| 512 QKV and input/output projections | Async completion (Section 3) + batched loading (Section 4) | Retain their respective budgets; remove the illustrated local displacement chains |
| Encoder/decoder 512 FFN and pooling | Async completion + per-entry resource budgets | FFN cap 128, actual 118, zero stack; pooling retains fixed-block code generation |
| ViT compute | Async completion and per-entry resource organization; shared spilling in selected attention | Eight attention calls use 16,400 bytes shared with capacity still 4 CTAs/SM |
| Scale input/output and resampling boundaries | Per-entry deferred loads, N64 reuse, budgets and shared spilling | 256 input/output use N64; selected downsampling adds shared spilling, upsampling keeps its tile |
| Temporal Pre | Lower register budget with local spills (Section 6) | 32 threads, 128 registers, 80-byte stack, 2,048 bytes shared |
| Temporal Post | Register budget + shared spilling (Section 7) | 32 threads, 128 registers, zero stack; 0 / 0-byte spill stores/loads, 2,560 bytes shared |
...
### 8.2 Module measurements and combination scope

| Measured group | Local time reduction | Measurement scope |
|---|---:|---|
...
| 256 chained, 12 calls | 17.85% / 18.92% | N64/128-budget combination against community |
| Temporal Pre | 6.60–9.82% | Local repeats on history frames against community |
| Temporal Post | 2.84–4.19% | Local repeats on history frames against community |
...
Section 7's incremental shared results use the preceding optimized selection as their baseline and cannot be added to this table.

<a id="runtime-integration"></a>

## 9. Runtime integration and general dimensions

### 9.1 Integration and function substitution
...
### 9.2 Textures, surfaces and history

The no-history initialization graph remains native; qualifying temporal contracts enable the corresponding Pre/Post routes. Texture objects, surface formats, sampling coordinates and history-update relationships follow the original calls.
...
### 9.3 Native dimensions and specialization
...
## 10. Validation methods and performance results

### 10.1 Experimental levels and statistics
...
### 10.3 Pure inference and game results
...
### 10.4 Added game-frame cost and external reference
...
### 10.5 Interpretation and next work
